...
def checklogin():
    ...
    path = request.path
    # 要求登录才能请求的路径
    urllist = [url_for('home.createblog'), url_for('home.bloginfo', pid=1)]
    if path in urllist:
        # 判断是否登录
...
def comment():
    ...
    res = '<script>alert("评论成功!");location.href="{ok}"</script>'.format(
        ok=url_for('home.bloginfo', pid=request.values['pid']))
    return res
...
def home():
    ...
    p = int(request.args.get('p', 1))
    # 获取博文数据  参数1 当前页码数  参数2 每页显示的数量
    posts = Posts.query.filter().paginate(p, 2)
    a = list(posts.iter_pages())
    b = len(a)
    data = {'posts': posts, 'a': a, 'b': b}
    # print(b)
